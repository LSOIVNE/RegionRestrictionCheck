...
local_isp4type=$(curl $curlArgs --referer https://ipinfo.io/ -s -4 --max-time 10 --user-agent "${UA_Browser}" "https://ipinfo.io/widget/demo/${local_ipv4}" | jq '.data.asn.type' | tr -d '"' &)
...
local_isp6type=$(curl $curlArgs --referer https://ipinfo.io/ -s -6 --max-time 10 --user-agent "${UA_Browser}" "https://ipinfo.io/widget/demo/${local_ipv6}" | jq '.data.asn.type' | tr -d '"' &)
...
function Global_UnlockTest() {
    ...
    wait
    local array=("Dazn:" "HotStar:" "Disney+:" "Netflix:" "YouTube Premium:" "Amazon Prime Video:" "TVBAnywhere+:" "iQyi Oversea:" "Viu.com:" "Tiktok" "YouTube CDN:" "Google" "YouTube Region:" "Netflix Preferred CDN:" "Spotify Registration:" "Steam Currency:" "Openai:")
    echo_Result ${result} ${array}
    echo "======================================="
}
...
function CheckV4() {
    if [[ "$language" == "e" ]]; then
        if [[ "$NetworkType" == "6" ]]; then
            ...
            echo "--------------------------------"
            echo -e " ${Font_SkyBlue}** Your Network Provider: ${local_isp4} (${local_ipv4_asterisk}) ['${local_isp4type}' IP in '${local_isp4country}']${Font_Suffix} "
            if [ -n  "$check4"  ]; then
                isv4=1
            else
        ...
        if [[ "$NetworkType" == "6" ]]; then
            ...
            check4=$(curl $curlArgs cloudflare.com/cdn-cgi/trace -4 -s 2>&1)
            echo "--------------------------------"
            echo -e " ${Font_SkyBlue}** 您的网络为: ${local_isp4} (${local_ipv4_asterisk}) ['${local_isp4type}' IP in '${local_isp4country}']${Font_Suffix} "
            if [ -n  "$check4"  ]; then
                isv4=1
            else
    ...
    if [[ "$language" == "e" ]]; then
        if [[ "$NetworkType" == "4" ]]; then
            ...
            if [ -n  "$check6"  ]; then
                ...
                echo "--------------------------------"
                echo -e " ${Font_SkyBlue}** Your Network Provider: ${local_isp6} (${local_ipv6_asterisk}) ['${local_isp6type}' IP in '${local_isp6country}']${Font_Suffix} "
                isv6=1
            else
        ...
        if [[ "$NetworkType" == "4" ]]; then
            ...
            if [ -n  "$check6"  ]; then
                ...
                echo "--------------------------------"
                echo -e " ${Font_SkyBlue}** 您的网络为: ${local_isp6} (${local_ipv6_asterisk}) ['${local_isp6type}' IP in '${local_isp6country}']${Font_Suffix} "
                isv6=1
            else
